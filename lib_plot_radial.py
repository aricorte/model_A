...
#    stds = np.std(matrix, 1)
    ...
    # x = range(n)
    plt.figure()
    plt.errorbar(radii, err, yerr=serrmed, c='k')
    if title:
        plt.title(title)
    if descr:
...
def plot_model(dirname, file_errors=None):
    """Opens all catalogues of single model and generates error bar plot."""
    ...
    for dir_ in dirs:
        path_radial = os.path.join(dir_, "radial.dat")
        if not os.path.isfile(path_radial):
            if file_errors:
                file_errors.write(dir_+"\n")
            print "WARNING: skipping %s" % dir_
        else:
            rads.append(np.loadtxt(path_radial))
            # print rads[-1]
    ...
    plot_thing(radii, err, serrmed, model_name, descr)  
    plt.savefig("%s-radial.png" % model_name)
    # plt.show()
